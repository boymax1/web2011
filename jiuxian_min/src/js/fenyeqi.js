...
Pagination.prototype.jinyong=function(){
    ...
    //判断当前页是否为第一页
    if(num2==1){
       divs[0].style.color='#eee'
       divs[2].style.color='#333'
    }
    //当前页为最一页时
    if(num2==this.default.pageInfo.totalpage){
       divs[0].style.color='#333'
       divs[2].style.color='#eee'
    }
...
Pagination.prototype.createText=function(){
    ...
    for(let attr in this.default.textInfo){
        ...
        //给新的div对象添加class属性值
        newDiv.className=attr+" page-active";
        //判断该键名是否为list对象
        if(attr == 'list'){
...
Pagination.prototype.showP=function(){
    ...
    if(totalpage2<5){
        ...
        if(pagenum2<5) {
        ...
        }else if(pagenum2>=5 && pagenum2<totalpage2-3){
            let newP=createP(1,pagenum2)
            this.list.appendChild(newP)
            var span = document.createElement('span')
            setCss(span,{
                "float": "left",
            ...
            span.innerHTML = '...'
            this.list.appendChild(span)
            for(let i=pagenum2-3;i<=pagenum2+3;i++){
                //创建p对象存放页码
                let newP=createP(i,pagenum2)
                //把当前创建好的p对象追加到this.list盒子中
                this.list.appendChild(newP)
            ...
            span.innerHTML = '...'
            this.list.appendChild(span)
            for(let i=pagenum2-3;i<=pagenum2+(totalpage2-pagenum2);i++){
                //创建p对象存放页码
                let newP=createP(i,pagenum2)
                //把当前创建好的p对象追加到this.list盒子中
                this.list.appendChild(newP)
...
Pagination.prototype.dongqilai=function(){
    ...
    this.ele.onclick=(e)=>{
        ...
        //判断点击的是否为下一页
        if(target.className.indexOf("next")!==-1 && this.default.pageInfo.pagenum!=this.default.pageInfo.totalpage){
            //修改当前页码
            this.default.pageInfo.pagenum=this.default.pageInfo.pagenum-0+1
            this.setStyle()
        }
        //判断点击的是否为上一页
        if(target.className.indexOf("prev")!==-1 && this.default.pageInfo.pagenum!=1){
            //修改当前页码
